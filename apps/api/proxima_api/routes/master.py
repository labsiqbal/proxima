...
def _as_int(value: Any) -> int:
    if isinstance(value, bool):
        raise HTTPException(
            status_code=422, detail="expected an integer, got a boolean"
        )
    try:
        return int(value)
...
def register(app, deps):
    ...
    def _identity(user: dict[str, Any]):
        try:
            ...
            code = (
                exc.code
                if isinstance(exc, MasterToolError)
                else "master_identity_inconsistent"
            )
            raise HTTPException(
                status_code=409,
    ...
    def _message_context(
        ...
        if target_mode == "explicit":
            ...
            target.get("container_id") is not None or target.get("area_id") is not None
        ):
            raise HTTPException(
                status_code=422,
    ...
    def create_master_message(
        ...
        with app.state.db_lock:
            ...
            try:
                ...
                if active:
                    raise HTTPException(
                        status_code=409, detail="Master is already working on a turn"
                    )
                ...
                target_container_id = (
                    requested_target.get("container_id")
                    if requested_target.get("mode") == "explicit"
                    else None
                )
                if target_container_id is not None:
                    ...
                    if focus["current_container_id"] != target_id:
                        ...
                        focus_changed = focus_changed or bool(focus.get("changed"))
        ...
        app.state.worker.add_event(
            ...
            None,
            "run.queued",
            {"runner": profile["runner_id"], "master": True},
        )
        return {
            "run_id": run_id,
        ...
        with app.state.db_lock:
            ...
            try:
                ...
                resolved_container_id = (
                    _as_int(container["id"]) if container is not None else None
                )
                active = conn.execute(
    ...
    def put_master_settings(
        ...
        if runner_id is not None:
            if not isinstance(runner_id, str) or not runner_is_selectable(runner_id):
                raise HTTPException(
                    status_code=422, detail="unknown Master backing runner"
                )
        ...
        for boolean_key in ("unattended", "tour_core_done"):
            if boolean_key in payload and not isinstance(payload[boolean_key], bool):
                raise HTTPException(
                    status_code=422, detail=f"{boolean_key} must be true or false"
                )
        token_value: int | None | object = ...
        if "budget_tokens" in payload:
            token_value = (
                None
                if payload["budget_tokens"] in (None, "")
                else _as_int(payload["budget_tokens"])
            )
        try:
            settings = app_settings.set_master_settings(
                db(),
                unattended=bool(payload["unattended"])
                if "unattended" in payload
                else None,
                budget_turns=_as_int(payload["budget_turns"])
                if "budget_turns" in payload
                else None,
                budget_wall_seconds=_as_int(payload["budget_wall_seconds"])
                if "budget_wall_seconds" in payload
                else None,
                budget_tokens=token_value,
                tour_core_done=bool(payload["tour_core_done"])
                if "tour_core_done" in payload
                else None,
            )
                ...
                json.dumps(
                    {
                        ...
                        if key != "budget_tokens" or value is not None
                    }
                ),
            ),
        )
    ...
    def preview_checkpoint_restore(
        job_id: int, checkpoint_id: int, user: dict[str, Any] = Depends(current_user)
    ):
    ...
    def restore_job_checkpoint(
        ...
        try:
            ...
            detail = str(exc)
            raise HTTPException(
                status_code=409 if "running" in detail else 422, detail=detail
            ) from exc
        ...
        if recovery_delivery and projection is not None:
            projection.safe_process_recovery_outbox(_as_int(recovery_delivery["id"]))
        if recovery_delivery:
            repair = (
                db()
                .execute(
                    "SELECT state, failure_code FROM task_recovery_outbox WHERE id = ?",
    ...
    def pin_job_checkpoint(
        ...
        if row["job_id"] != job_id:
            raise HTTPException(status_code=404, detail="checkpoint not found for job")
        db().execute(
            "UPDATE job_checkpoints SET pinned = ? WHERE id = ?",
            (1 if payload.get("pinned", True) else 0, checkpoint_id),
        )
        return checkpoint_payload(
            db()
            .execute("SELECT * FROM job_checkpoints WHERE id = ?", (checkpoint_id,))
            .fetchone()
        )
    ...
    def _message_journal(message_id: int, user: dict[str, Any]):
        ...
        if row["mode"] != "chat" or not row["project_path"]:
            raise HTTPException(
                status_code=409,
                detail="turn restore is available only for project Chat sessions",
            )
        return row

    @app.get("/api/chat/messages/{message_id}/restore-turn")
    def preview_turn_restore(
        message_id: int, user: dict[str, Any] = Depends(current_user)
    ):
    ...
    def restore_chat_turn(
        ...
        try:
            ...
            detail = str(exc)
            raise HTTPException(
                status_code=409 if "Master" in detail or "active" in detail else 422,
                detail=detail,
            ) from exc
        return result
    ...
    def _attention_items(user: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for row in (
            ...
            final_simple = (
                ...
                and not row["has_worktree"]
                and row["engine"] != "graph"
            )
            ...
            items.append(
                {
                    "id": f"job:{row['id']}",
                    "kind": kind,
                    "title": f"{row['title']} needs review",
                    "target": {
                        "view": "task",
                        "job_id": row["id"],
                        "engine": row["engine"],
                        "origin_master_session_id": row["origin_master_session_id"],
                    },
        ...
        if read_script:
            for row in (
                ...
                items.append(
                    {
                        "id": f"script:{row['job_id']}:{row['node_id']}",
                        "kind": "script_trust",
    ...
    def act_attention(
        ...
        if item_id.startswith("attention:"):
            ...
            if not row["inline_ok"]:
                raise HTTPException(
                    status_code=400,
                    detail="this item must be handled on its linked surface",
                )
            ...
            if row["kind"] == "permission_job":
                ...
                option = next(
                    (o for o in options if str(o.get("kind", "")).startswith(wanted)),
                    None,
                )
                if not option or not app.state.worker.resolve_permission(
                    _as_int(target.get("run_id")),
                    str(target.get("request_id")),
                    str(option.get("optionId")),
                ):
                    raise HTTPException(
                        status_code=409, detail="permission request is no longer active"
                    )
            else:
        ...
        if item_id.startswith("job:"):
            ...
            current = next(
                (item for item in _attention_items(user) if item["id"] == item_id), None
            )
            if not current or not current["inline_ok"]:
                raise HTTPException(
                    status_code=400, detail="this review must be handled in Tasks"
                )
            ...
            if action == "approve" and approve_job:
                ...
                reject_job(
                    job_id, JobRejectRequest(reason="Rejected from Attention"), user
                )
        ...
        if item_id.startswith("script:"):
            ...
            if not current or not current["inline_ok"]:
                raise HTTPException(
                    status_code=409, detail="script approval is no longer active"
                )
            approve_script = getattr(app.state, "master_approve_node_script", None)
            if not approve_script:
                raise HTTPException(
                    status_code=409, detail="script approval service is unavailable"
                )
            approve_script(
                job_id,
                node_id,
                GraphScriptApproveRequest(
                    expected_sha256=str(current["target"]["sha256"])
                ),
                user,
            )
            return {"ok": True, "id": item_id, "action": action}
        if item_id.startswith("satpam:"):
            ...
            row = (
                db()
                .execute(
                    "SELECT * FROM satpam_interventions WHERE id=? AND status='pending'",
                    (intervention_id,),
                )
                .fetchone()
            )
            ...
            if action == "approve":
                try:
                    app.state.worker.satpam.execute_restart(
                        row["job_id"], intervention_id
                    )
                except satpam.SatpamRestartError as exc:
                    raise HTTPException(status_code=409, detail=str(exc)) from exc
            elif action == "dismiss":
                db().execute(
                    "UPDATE satpam_interventions SET status='dismissed', resolved_at=CURRENT_TIMESTAMP WHERE id=?",
                    (intervention_id,),
                )
            else:
                raise HTTPException(status_code=400, detail="action is not available")
