...
def test_chat_send_accepts_non_hermes_runner(tmp_path):
    ...
    res = client.post(
        ...
        json={"message": "hello", "runner_id": "claude-code"},
    )
...
def test_masterplan_supersedes_blocked_goal_so_followups_stay_masterplan(tmp_path):
    app = create_app(
        {
            "database_path": str(tmp_path / "proxima.db"),
            "workspace_root": str(tmp_path / "workspace"),
    ...
    started = client.post(
        "/api/chat/send", headers=headers, json={"message": "hello"}
    ).json()
    session_id = started["session_id"]
    with app.state.db_lock:
        app.state.db.execute(
            "UPDATE sessions SET goal_text = ?, goal_status = 'blocked' WHERE id = ?",
            ("ship the CLI", session_id),
        )

    masterplan = client.post(
        f"/api/sessions/{session_id}/runs",
        headers=headers,
        json={"message": "/masterplan build a durable CLI"},
    )
    assert masterplan.status_code == 202, masterplan.text
    with app.state.db_lock:
        goal_status = app.state.db.execute(
            "SELECT goal_status FROM sessions WHERE id = ?", (session_id,)
        ).fetchone()["goal_status"]
    assert goal_status == "cancelled"

    followup = client.post(
        f"/api/sessions/{session_id}/runs",
        headers=headers,
        json={"message": "here is my clarification answer"},
    ).json()
    run = client.get(f"/api/runs/{followup['run_id']}", headers=headers).json()
    assert "[Proxima goal mode]" not in run["prompt"]
    assert run["prompt"] == "here is my clarification answer"
...
def test_empty_masterplan_command_prompts_agent_to_ask_for_idea(tmp_path):
    ...
    response = client.post(
        "/api/chat/send", headers=headers, json={"message": "/masterplan"}
    )
...
def test_chat_send_hermes_enqueues_async_run(tmp_path):
    ...
    events = client.get(
        f"/api/sessions/{body['session_id']}/events",
        headers={"Authorization": f"Bearer {token}"},
    )
...
def test_main_chat_image_request_creates_artifact_first_result(tmp_path, monkeypatch):
    ...
    res = client.post(
        ...
        json={
            "project_slug": slug,
            "message": "/image neon robot mascot for onboarding",
        },
    )
...
def test_thin_image_brief_asks_question_form_instead_of_generating(
    ...
    res = client.post(
        "/api/chat/send",
        headers=headers,
        json={"project_slug": "demo", "message": "/image"},
    )
    assert res.status_code == 202, res.text
    body = res.json()
...
def test_thin_design_brief_asks_form_but_rich_brief_creates_draft(tmp_path):
    ...
    thin = client.post(
        "/api/chat/send",
        headers=headers,
        json={"project_slug": "demo", "message": "/design"},
    ).json()
    assert thin["media_action"] == "image-studio_ask"
    ask_msg = client.get(
        f"/api/sessions/{thin['session_id']}/messages", headers=headers
...
def test_run_can_store_display_message_separate_from_prompt(tmp_path):
    ...
    session_id = client.post(
        "/api/sessions", headers=headers, json={"title": "iterate"}
    ).json()["id"]
    run_id = client.post(
        f"/api/sessions/{session_id}/runs",
        headers=headers,
        json={
            "message": "long internal prompt",
            "display_message": "Run step 1: Smoke",
        },
    ).json()["run_id"]

    msg = client.get(f"/api/sessions/{session_id}/messages", headers=headers).json()[
        "messages"
    ][0]
...
def test_collaboration_settings_roundtrip_and_validation(tmp_path):
    ...
    assert client.get("/api/settings/collaboration", headers=headers).json() == {
        "brainstorm_agents": 3,
        "debate_rounds": 2,
    }
    saved = client.put(
        ...
        headers=headers,
        json={"brainstorm_agents": 2, "debate_rounds": 4},
    )
    ...
    assert (
        client.put(
            "/api/settings/collaboration",
            headers=headers,
            json={"brainstorm_agents": 4, "debate_rounds": 2},
        ).status_code
        == 400
    )
...
def test_run_prompt_mode_starts_brainstorm_collaboration(tmp_path):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    session_id = client.post(
        "/api/sessions", headers=headers, json={"title": "brainstorm"}
    ).json()["id"]
    ...
    collab = app.state.db.execute(
        "SELECT * FROM prompt_collaborations WHERE parent_run_id = ?", (body["run_id"],)
    ).fetchone()
    children = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND id != ? ORDER BY id",
        (collab["id"], body["run_id"]),
    ).fetchall()
    events = client.get(f"/api/sessions/{session_id}/events", headers=headers).json()[
        "events"
    ]
...
def test_brainstorm_uses_configured_agent_count(tmp_path):
    ...
    token = client.post("/auth/auto").json()["token"]
    headers = {"Authorization": f"Bearer {token}"}
    user_id = app.state.db.execute(
        "SELECT id FROM users WHERE username = 'bob'"
    ).fetchone()["id"]
    app.state.db.execute(
        "INSERT INTO profiles(user_id, slug, name, runner_id, hermes_home) VALUES (?, 'p2', 'Agent Two', 'codex', ?)",
        (user_id, str(tmp_path / "h2")),
    )
    app.state.db.execute(
        "INSERT INTO profiles(user_id, slug, name, runner_id, hermes_home) VALUES (?, 'p3', 'Agent Three', 'claude-code', ?)",
        (user_id, str(tmp_path / "h3")),
    )
    client.put(
        "/api/settings/collaboration",
        headers=headers,
        json={"brainstorm_agents": 2, "debate_rounds": 2},
    )
    session_id = client.post(
        "/api/sessions", headers=headers, json={"title": "brainstorm"}
    ).json()["id"]
...
def test_debate_uses_configured_round_count(tmp_path):
    ...
    client.put(
        "/api/settings/collaboration",
        headers=headers,
        json={"brainstorm_agents": 3, "debate_rounds": 3},
    )
    session_id = client.post(
        "/api/sessions", headers=headers, json={"title": "debate"}
    ).json()["id"]
    parent_id = client.post(
        f"/api/sessions/{session_id}/runs",
        headers=headers,
    ...
    stance = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND kind = 'collab_debate_stance'",
        (collab["id"],),
    ).fetchone()
    app.state.db.execute(
        "UPDATE runs SET status = 'running' WHERE id = ?", (stance["id"],)
    )
    app.state.worker._complete_collaboration_run(
        dict(
            app.state.db.execute(
                "SELECT * FROM runs WHERE id = ?", (stance["id"],)
            ).fetchone()
        ),
        "Opening argument.",
        "stop",
    )
    rebuttal = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND kind = 'collab_debate_rebuttal'",
        (collab["id"],),
    ).fetchone()
    app.state.db.execute(
        "UPDATE runs SET status = 'running' WHERE id = ?", (rebuttal["id"],)
    )
    app.state.worker._complete_collaboration_run(
        dict(
            app.state.db.execute(
                "SELECT * FROM runs WHERE id = ?", (rebuttal["id"],)
            ).fetchone()
        ),
        "Rebuttal argument.",
        "stop",
    ...
    counter = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND kind = 'collab_debate_counter_rebuttal'",
        (collab["id"],),
    ).fetchone()
    ...
    app.state.db.execute(
        "UPDATE runs SET status = 'running' WHERE id = ?", (counter["id"],)
    )
    app.state.worker._complete_collaboration_run(
        dict(
            app.state.db.execute(
                "SELECT * FROM runs WHERE id = ?", (counter["id"],)
            ).fetchone()
        ),
        "Counter-rebuttal.",
        "stop",
    ...
    synth = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND kind = 'collab_debate_synthesis'",
        (collab["id"],),
    ).fetchone()
...
def test_brainstorm_collaboration_synthesizes_single_visible_message(tmp_path):
    ...
    child = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND kind = 'collab_brainstorm_child'",
        (collab["id"],),
    ).fetchone()

    app.state.db.execute(
        "UPDATE runs SET status = 'running' WHERE id = ?", (child["id"],)
    )
    assert (
        app.state.worker._complete_collaboration_run(
            dict(
                app.state.db.execute(
                    "SELECT * FROM runs WHERE id = ?", (child["id"],)
                ).fetchone()
            ),
            "Try a compact sidecar.",
            "stop",
        )
        is True
    )
    ...
    synth = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND kind = 'collab_brainstorm_synthesis'",
        (collab["id"],),
    ).fetchone()
    ...
    app.state.db.execute(
        "UPDATE runs SET status = 'running' WHERE id = ?", (synth["id"],)
    )
    assert (
        app.state.worker._complete_collaboration_run(
            dict(
                app.state.db.execute(
                    "SELECT * FROM runs WHERE id = ?", (synth["id"],)
                ).fetchone()
            ),
            "Use the sidecar and keep actions compact.",
            "stop",
        )
        is True
    )
    ...
    assert (
        client.get(f"/api/runs/{parent_id}", headers=headers).json()["status"]
        == "completed"
    )
    events = client.get(f"/api/sessions/{session_id}/events", headers=headers).json()[
        "events"
    ...
    completed_cards = [
        e for e in events if e["type"] == "collaboration.child.completed"
    ]
...
def test_brainstorm_child_failure_fails_parent_not_cancelled(tmp_path):
    ...
    child = app.state.db.execute(
        "SELECT * FROM runs WHERE collaboration_id = ? AND id != ?",
        (collab["id"], parent_id),
    ).fetchone()
    app.state.db.execute(
        "UPDATE runs SET status = 'running' WHERE id = ?", (child["id"],)
    ...
    assert (
        client.get(f"/api/runs/{child['id']}", headers=headers).json()["status"]
        == "failed"
    )
    assert (
        app.state.db.execute(
...
def test_cancel_brainstorm_parent_emits_child_cancelled_card(tmp_path):
    ...
    cancelled_card = next(
        e for e in events if e["type"] == "collaboration.child.cancelled"
    )
...
def test_workflow_iterate_instant_result_completes_without_worker(tmp_path):
    ...
    rejected = client.post(
        ...
        json={
            "message": "prompt",
            "display_message": "Run step 1",
            "instant_result": "test received",
        },
    )
    ...
    app.state.db.execute(
        "UPDATE sessions SET workflow_id = ? WHERE id = ?", (_wf_id, session_id)
    )
    res = client.post(
        f"/api/sessions/{session_id}/runs",
        headers=headers,
    ...
    assert (
        client.get(f"/api/runs/{body['run_id']}", headers=headers).json()["status"]
        == "completed"
    )
    messages = client.get(
        f"/api/sessions/{session_id}/messages", headers=headers
...
def test_delete_completed_run_removes_result_messages_and_events(tmp_path):
    ...
    assert (
        client.get(f"/api/sessions/{session_id}/messages", headers=headers).json()[
            "messages"
        ]
        == []
    )
    assert (
        client.get(f"/api/sessions/{session_id}/events", headers=headers).json()[
            "events"
        ]
        == []
    )
...
def test_delete_session_artifact_scrubs_stale_message_and_event_links(tmp_path):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    client.post(
        "/api/projects", headers=headers, json={"slug": "alpha", "name": "Alpha"}
    )
    session_id = client.post(
        "/api/sessions",
        headers=headers,
        json={"title": "iterate", "project_slug": "alpha"},
    ).json()["id"]
    _wf_id = app.state.db.execute("INSERT INTO workflows(name) VALUES ('wf')").lastrowid
    app.state.db.execute(
        "UPDATE sessions SET workflow_id = ? WHERE id = ?", (_wf_id, session_id)
    ...
    app.state.db.execute(
        "UPDATE events SET payload = ? WHERE run_id = ? AND type = 'message.complete'",
        (json.dumps({"output_links": [link]}), run_id),
    )

    res = client.delete(
        f"/api/sessions/{session_id}/artifacts?path=artifacts%2Fout.md", headers=headers
    )
    ...
    assert (
        app.state.db.execute(
            "SELECT output_links FROM messages WHERE run_id = ?", (run_id,)
        ).fetchone()["output_links"]
        == "[]"
    )
...
def test_cancel_completed_run_is_idempotent_without_cancel_event(tmp_path):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    session_id = client.post(
        "/api/sessions", headers=headers, json={"title": "done"}
    ).json()["id"]
    run_id = client.post(
        f"/api/sessions/{session_id}/runs", headers=headers, json={"message": "hello"}
    ).json()["run_id"]
    app.state.db.execute(
...
def test_cancel_completed_run_does_not_cancel_new_queued_run(tmp_path):
    ...
    old_run_id = client.post(
        f"/api/sessions/{session_id}/runs", headers=headers, json={"message": "old"}
    ).json()["run_id"]
    app.state.db.execute(
        "UPDATE runs SET status = 'completed', finished_at = CURRENT_TIMESTAMP WHERE id = ?",
        (old_run_id,),
    ...
    new_run_id = client.post(
        f"/api/sessions/{session_id}/runs", headers=headers, json={"message": "new"}
    ).json()["run_id"]
    ...
    assert res.status_code == 200
    assert (
        client.get(f"/api/runs/{old_run_id}", headers=headers).json()["status"]
        == "completed"
    )
    assert (
        client.get(f"/api/runs/{new_run_id}", headers=headers).json()["status"]
        == "queued"
    )
...
def test_permission_response_rejected_for_terminal_run(tmp_path):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    session_id = client.post(
        "/api/sessions", headers=headers, json={"title": "permission"}
    ).json()["id"]
    run_id = client.post(
        f"/api/sessions/{session_id}/runs", headers=headers, json={"message": "hello"}
    ).json()["run_id"]
...
def test_permission_response_routes_to_matching_active_run(tmp_path):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    sid_a = client.post(
        "/api/sessions", headers=headers, json={"title": "permission a"}
    ).json()["id"]
    sid_b = client.post(
        "/api/sessions", headers=headers, json={"title": "permission b"}
    ).json()["id"]
    run_a = client.post(
        f"/api/sessions/{sid_a}/runs", headers=headers, json={"message": "a"}
    ).json()["run_id"]
    run_b = client.post(
        f"/api/sessions/{sid_b}/runs", headers=headers, json={"message": "b"}
    ).json()["run_id"]
    app.state.db.execute(
        "UPDATE runs SET status = 'running' WHERE id IN (?, ?)", (run_a, run_b)
    )
    spy_a = PermissionSpy()
    spy_b = PermissionSpy()
...
def test_session_runs_endpoint_intercepts_media_prompts(tmp_path, monkeypatch):
    ...
    client.post("/api/projects", headers=headers, json={"slug": "demo", "name": "Demo"})
    sid = client.post(
        "/api/sessions", headers=headers, json={"title": "ugc", "project_slug": "demo"}
    ).json()["id"]
    calls: list[str] = []
    ...
    res = client.post(
        ...
        headers=headers,
        json={"message": "/image ugc product teaser cinematic"},
    )
    ...
    with app.state.db_lock:
        kind = app.state.db.execute(
            "SELECT kind FROM runs WHERE id = ?", (body["run_id"],)
        ).fetchone()["kind"]
        msgs = [
            r["role"]
...
def test_image_file_mention_reaches_generation_provider_as_pixels(
    tmp_path, monkeypatch
):
    app = _media_app(tmp_path)
    client = TestClient(app)
    ...
    def fake_generate(
        provider_id, key, *, prompt, image_bytes=None, image_mime=None, **kwargs
    ):
...
def test_media_run_failure_lands_in_thread(tmp_path, monkeypatch):
    ...
    res = client.post(
        f"/api/sessions/{sid}/runs",
        headers=headers,
        json={"message": "/image sufficiently rich brief"},
    )
    assert res.status_code == 202
    body = res.json()
...
def test_cancelled_media_run_stays_cancelled_when_provider_finishes(
    tmp_path, monkeypatch
):
    app = _media_app(tmp_path)
    client = TestClient(app)
    ...
    sid = client.post(
        "/api/sessions",
        headers=headers,
        json={"title": "image", "project_slug": "demo"},
    ).json()["id"]
    ...
    while time.time() < deadline:
        row = app.state.db.execute(
            "SELECT status FROM runs WHERE id = ?", (created["run_id"],)
        ).fetchone()
...
def test_session_runs_media_skips_collab_and_instant(tmp_path, monkeypatch):
    ...
    res = client.post(
        ...
        headers=headers,
        json={
            "message": "/image product teaser cinematic",
            "prompt_mode": "brainstorm",
        },
    )
    assert res.status_code == 202
...
def test_natural_language_media_phrases_go_to_the_agent(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(image_providers, "generate", explode)
    for message in (
        "test generate video ugc durasi 12 detik",
        "buat gambar thumbnail buat launching",
    ):
        res = client.post(
            f"/api/sessions/{sid}/runs", headers=headers, json={"message": message}
        )
        assert res.status_code == 202
...
def test_session_runs_without_project_falls_back_to_agent(tmp_path, monkeypatch):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    sid = client.post(
        "/api/sessions", headers=headers, json={"title": "no project"}
    ).json()["id"]
    ...
    res = client.post(
        f"/api/sessions/{sid}/runs",
        headers=headers,
        json={"message": "/image ugc product demo"},
    )
    assert res.status_code == 202
    assert res.json()["status"] == "queued"  # normal agent run
...
def test_image_studio_command_creates_design_draft(tmp_path):
    ...
    client.post("/api/projects", headers=headers, json={"slug": "demo", "name": "Demo"})
    sid = client.post(
        "/api/sessions", headers=headers, json={"title": "ig", "project_slug": "demo"}
    ).json()["id"]

    res = client.post(
        f"/api/sessions/{sid}/runs",
        headers=headers,
        json={"message": "/image-studio carousel promo snacktray lawson"},
    )
    ...
    with app.state.db_lock:
        drow = app.state.db.execute(
            "SELECT id, mode FROM sessions WHERE id = ?", (scene["sessionId"],)
        ).fetchone()
        rrow = app.state.db.execute(
            "SELECT status, prompt FROM runs WHERE session_id = ? ORDER BY id DESC LIMIT 1",
            (scene["sessionId"],),
        ).fetchone()
    assert drow and drow["mode"] == "design"
    assert rrow and rrow["status"] == "queued"
    assert (
        "carousel promo snacktray lawson" in rrow["prompt"]
        and "Current scene" in rrow["prompt"]
    )
