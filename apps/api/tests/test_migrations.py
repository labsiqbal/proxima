...
def test_v48_retains_published_successor_ordering_gap(tmp_path: Path):
    ...
    assert run_migrations(conn, str(db_path)) == [47, 48, 49, 50, 51]
    assert run_migrations(conn, str(db_path)) == []
    assert current_version(conn) == 51
    gap = dict(
        conn.execute(
...
def test_v50_preserves_multiple_delivered_v47_markers_before_v48(
    ...
    init_db(conn)
    assert run_migrations(conn, str(db_path)) == [49, 50, 51]
    assert run_migrations(conn, str(db_path)) == []
    after_corrections = [
        tuple(row)
        for row in conn.execute(
...
def test_v50_recovers_multiple_delivered_markers_from_v48_history(
    ...
    assert run_migrations(conn, str(db_path)) == [50, 51]
    assert [
        int(row["id"])
        for row in conn.execute(
