...
export function buildDesignPrompt(scene: Scene, selected: { id: string; type: string; label: string } | null, instruction: string): string {
  const lines = [
    ...
    'To include a PHOTO or ILLUSTRATION, add an image layer whose src is "gen:<a short image-generation prompt>" — e.g. "gen:a green plastic bottle, studio product shot, soft shadow, white background". The studio will generate the real image with AI and swap it in. NEVER invent file paths or URLs for images; only use gen: prompts (or an existing asset path the user already has).',
    '',
    'Current scene:',
    '```json',
