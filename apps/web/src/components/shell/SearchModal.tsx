...
export function SearchModal(props: {
  ...
  const openSession = (id: number, taskId: number | null, title = '') => {
  ...
  const total = res.projects.length + chats.length + res.tasks.length + messages.length
  return <div className="modal-scrim" onClick={props.onClose}><div className="modal-card search-modal" onClick={e => e.stopPropagation()}>
    ...
    <div className="search-results">
      ...
      {chats.length > 0 && <div className="search-group"><p className="eyebrow">Chats</p>{chats.map(c => <button className="search-item" key={'c' + c.id} onClick={() => openSession(c.id, c.task_id, c.title)}><strong>{c.title}</strong></button>)}</div>}
      {messages.length > 0 && <div className="search-group"><p className="eyebrow">Messages</p>{messages.map((m, i) => <button className="search-item" key={'m' + i} onClick={() => openSession(m.session_id, m.task_id, m.session_title)}><strong>{m.session_title}</strong><small>{m.role}: {m.snippet}</small></button>)}</div>}
    </div>
  </div></div>
}
