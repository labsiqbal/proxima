  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: "color-mix(in srgb, var(--proxima-video-text, #111827) 12%, transparent)",
          height: "3px",
        }}
      >
        ...
        <div
          ...
          style={{
            ...
            boxShadow:
              "0 0 0 3px color-mix(in srgb, var(--hf-accent, #3CE6AC) 16%, transparent), 0 1px 4px rgba(15,23,42,0.18)",
          }}
        />
      </div>
    </div>
...
export const PlayerControls = memo(function PlayerControls({
  ...
  return (
    <div
      ...
      aria-disabled={disabled || undefined}
      style={{
        borderTop: "1px solid var(--proxima-video-border-soft, rgba(15,23,42,0.08))",
    ...
    >
      <Tooltip label={isPlaying ? "Pause" : "Play"}>
        <button
          ...
          className="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg border border-studio-accent/25 text-studio-accent disabled:opacity-40 disabled:pointer-events-none transition-colors hover:bg-studio-accent/10"
          style={{
            background: "color-mix(in srgb, var(--hf-accent, #3CE6AC) 10%, var(--proxima-video-surface, #fff))",
          }}
        >
      ...
      >
        <button
          ...
          style={{ color: "var(--proxima-video-muted, #4b5563)", cursor: "pointer" }}
        >
          <span ref={timeDisplayRef}>{formatTime(0)}</span>
          {timeDisplayMode === "time" ? (
            <>
              <span style={{ color: "var(--proxima-video-faint, #8a94a6)", margin: "0 2px" }}>
                /
              </span>
              <span style={{ color: "var(--proxima-video-muted, #4b5563)" }}>
                {formatTime(duration)}
              </span>
            </>
          ) : null}
