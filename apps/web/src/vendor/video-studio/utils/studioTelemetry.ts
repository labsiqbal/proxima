...
function getDistinctId(): string {
  ...
  try {
    const stored = localStorage.getItem("proxima.videoStudio.anonymousId");
    if (stored) {
      distinctId = stored;
      return stored;
    }
  ...
  try {
    localStorage.setItem("proxima.videoStudio.anonymousId", distinctId);
  } catch {
...
function isEnabled(): boolean {
  try {
    return localStorage.getItem("proxima.videoStudio.telemetryDisabled") !== "1";
  } catch {
    return true;
  }
}
