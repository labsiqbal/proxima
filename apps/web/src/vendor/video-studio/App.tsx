...
export function StudioApp() {
  ...
  const renderToastStateRef = useRef<Map<string, string>>(new Map());
  useEffect(() => {
    for (const job of renderQueue.jobs) {
      const prev = renderToastStateRef.current.get(job.id);
      if (prev === job.status) continue;
      renderToastStateRef.current.set(job.id, job.status);
      if (job.status === "rendering") showToast(`Rendering ${job.filename}...`, "info");
      if (job.status === "complete") showToast(`Render complete: ${job.filename}`, "info");
      if (job.status === "failed") showToast(job.error ? `Render failed: ${job.error}` : "Render failed", "error");
    }
  }, [renderQueue.jobs, showToast]);
          ...
          <PanelLayoutProvider value={panelLayout}>
            <FileManagerProvider value={fileManager}>
              <DomEditProvider value={domEditSession}>
                <div
                  className="proxima-studio-root flex flex-col h-full w-full relative"
