...
import type { FileRootSide } from './files'
...
export function projectFs(
  ...
  return {
    list: ref => files.listTree(token, slug, scoped(base, ref), rootSide),
    read: ref => files.readFile(token, slug, scoped(base, ref), rootSide),
    write: (ref, content) => files.writeFile(token, slug, scoped(base, ref), content, rootSide),
    mkdir: ref => files.mkdir(token, slug, scoped(base, ref), rootSide),
    rename: (from, to) => files.renamePath(token, slug, scoped(base, from), scoped(base, to), rootSide),
    remove: ref => files.deletePath(token, slug, scoped(base, ref), rootSide),
  }
}
