...
export type FileRootSide = 'virtual' | 'container'
...
const rootSideQuery = (rootSide: FileRootSide) =>
  rootSide === 'container' ? '&root_side=container' : ''
...
export const listTree = (
  ...
  api<{ path: string; target?: FileTarget; entries: FileEntry[] }>(
    `/api/projects/${slug}/tree?${refQuery(ref)}${rootSideQuery(rootSide)}`,
    token,
  )
...
export const readFile = (
  ...
  api<{ path: string; target?: FileTarget; content: string }>(
