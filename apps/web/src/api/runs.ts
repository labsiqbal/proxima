...
export const listEvents = (token: string, sessionId: number, afterId = 0, signal?: AbortSignal) => api<{ events: RunEvent[] }>(`/api/sessions/${sessionId}/events?after_id=${afterId}`, token, { signal })
