...
export const runnerCapabilities = (token: string, runnerId: string, opts?: { rescan?: boolean }) =>
  api<RunnerCapabilities>(
    `/api/runners/${runnerId}/capabilities${opts?.rescan ? '?rescan=1' : ''}`,
    token,
  )

export const rescanRunnerCapabilities = (token: string, runnerId: string) =>
  api<RunnerCapabilities>(`/api/runners/${runnerId}/capabilities/rescan`, token, { method: 'POST' })
