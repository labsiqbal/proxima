...
export function App() {
  ...
  return (
    ...
    >
      ...
      {view === 'workflows' && <React.Suspense fallback={<ViewFallback label="Loading workflows..." />}><WorkflowsScreen mode={workflowMode} onModeChange={setWorkflowMode} advancedContent={features.workflowGraph ? <GraphScreen token={token} projects={projects} activeProject={activeProject} onActiveProject={setActiveProject} profileId={activeProfile?.id ?? null} pendingDraft={pendingGraphDraft} onDraftConsumed={() => setPendingGraphDraft(null)} pendingJobId={pendingGraphJob} onPendingConsumed={() => setPendingGraphJob(null)} /> : undefined} token={token} projects={projects} activeProject={activeProject} onActiveProject={setActiveProject} features={features} profiles={profiles} activeProfile={activeProfile} onOpenJob={openTask} draft={pendingDraft} onDraftConsumed={() => setPendingDraft(null)} /></React.Suspense>}
