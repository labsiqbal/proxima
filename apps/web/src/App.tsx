...
export function App() {
  ...
  const [workflowMode, setWorkflowMode] = React.useState<'sequential' | 'advanced' | 'scheduled'>('sequential')
  ...
  const goView = (v: View) => { clearTaskHash(); clearPendingNavigation(); if (v === 'workflows') setWorkflowMode('sequential'); setView(viewEnabled(v) ? v : 'home') }
  ...
  return (
    ...
    >
      ...
      {view === 'workflows' && <React.Suspense fallback={<ViewFallback label="Loading workflows..." />}><WorkflowsScreen mode={workflowMode} onModeChange={setWorkflowMode} advancedContent={features.workflowGraph ? <GraphScreen token={token} projects={projects} activeProject={activeProject} onActiveProject={setActiveProject} profiles={profiles} profileId={activeProfile?.id ?? null} pendingDraft={pendingGraphDraft} onDraftConsumed={() => setPendingGraphDraft(null)} pendingJobId={pendingGraphJob} onPendingConsumed={() => setPendingGraphJob(null)} /> : undefined} token={token} projects={projects} activeProject={activeProject} onActiveProject={setActiveProject} features={features} profiles={profiles} activeProfile={activeProfile} onOpenJob={openTask} draft={pendingDraft} onDraftConsumed={() => setPendingDraft(null)} /></React.Suspense>}
