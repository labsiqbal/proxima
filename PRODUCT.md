...
Self-hosting **small teams and startups** (and solo operators) who run a mix of **human + AI-agent** work. They install Proxima on their own machine/server and reach it privately (LAN, Tailscale, or a Cloudflare-tunneled subdomain). They are mid-technical at most — comfortable installing a tool, but they want the cockpit itself to be effortless. They are *in a task*: starting a chat, running a workflow, reviewing an agent's output, checking what's scheduled. The owner is often non-technical and delegates the actual building to agents — so the UI must make agent work legible and controllable without jargon.

## Product Purpose

Proxima is a **self-hosted control plane / cockpit** for human + AI agent teams: one private place to **chat** with agents, run repeatable **workflows** (recipes of steps, runnable now or on a cron schedule), watch each step execute and **review/approve** results, browse the project's **files & wiki**, and produce real artifacts. It is runner-agnostic (drives Hermes/Claude/Codex/Gemini over ACP) and BYO-AI (no per-seat SaaS). Success = the owner trusts it to run real work end-to-end, sees exactly what the agents did, and never feels they've left "their own machine." Headed toward **open source** (each user self-hosts for their own trusted team).
