...
It is anchored to the authoritative [ADR index](adr/README.md) records that were
accepted at gate time. It records the six ordered build gates, security
invariants, APIs, migrations, UI, runner conformance, graph isolation, and fault
cases. The gates have since passed: Master ships enabled by default (ADR-0039).
The safe-update boundaries this matrix originally covered (groups 14-16) were
retired when the safe-updater stack was deleted (prune A1, ADR-0041); their
rows below are kept as historical acceptance context, with the evidence in git
history.
...
| [ADR-0008](adr/0008-external-safe-update-authority.md) | Candidate releases never own update authority or production switching. | Historical: the gate passed against `apps/safe_updater/` and its disposable-fixture tests, all since deleted (prune A1); ADR-0008 is superseded by ADR-0041 and the evidence lives in git history. |
...
| 6. External candidate and fault gates | Build, clone migration, candidate probe, and switch or rollback faults are proven only in disposable controller fixtures. | Historical: ADR-0008 and the groups 14-16 candidate, controller, fence, journal, rollback, and process-containment tests - passed at gate time, deleted with the stack (prune A1, ADR-0041). |
...
| Candidate proof cannot touch production (historical) | Passed at gate time against the candidate sandbox, Bubblewrap isolation, and path validation of the deleted safe-updater stack (prune A1, ADR-0041); evidence in git history. |
| Fault recovery is fixture-proven (historical) | Passed at gate time against the deleted controller tests (journal replay, interruption, rollback, breaker, adapter refusal); evidence in git history (prune A1, ADR-0041). |
...
[`scripts/browser-harness/browser-scenarios.json`](../scripts/browser-harness/browser-scenarios.json)
is executed by the tracked Chrome DevTools driver
[`browser.py`](../scripts/browser-harness/browser.py). It asserts:
...
It does not replace live data or control a real service. The source-controlled
...
entirely; updating is a manual `git pull` plus a service restart (ADR-0041).
