...
`AppShell` owns the two status disclosure signals and applies their union to both
the popup and toast availability boundaries, preventing saturated mobile sheets
from sharing pixels with either Master overlay. It persists a left/right corner preference and
...
`AppShell` retains the persisted left navigation width/collapse state, mobile drawer, search, Attention, and account actions, and owns the right **`ToolDock`** (Terminal/Files/Preview as overlay panels). Desktop status controls remain in the top-bar cluster; a media-query subscriber mounts the same status controls into mobile header layout without duplicate polling. Running and Attention report their disclosure state to `AppShell`, which withholds the Master popup and toast region while either is open. The mobile header reserves an Attention slot and changes narrow Work widths to two rows, so Attention appearing or disappearing cannot move Search or New chat. Its shared height token includes the rendered row, switch, gap, padding, and border geometry. The header stacking layer paints above the rail and ordinary content, while fixed narrow status popovers reserve the tool-rail width plus an outer gap. Shared shell hit-target, focus-ring, label, and accent-text tokens provide the minimum target and 4.5:1 contrast contracts. There is a single workspace: `Sidebar` renders one flow-ordered navigation (Chat, Master, Tasks, Workflows, Archive, gated Design) and the default landing view is `chat`. Session-kind metadata separately declares global-search visibility: Chat and Design sessions are searchable, while Master's hidden system thread is excluded so structured product-tool calls never leak into owner-facing results. Terminal moved out of the view routing into the ToolDock, which mounts it on first open and then hides rather than unmounts it, preserving PTYs; Files reuses `WorkspaceTree`+`FileEditor` over `projectFs`, and Preview reuses `AppRunner`. The dock accepts an availability boundary from `App`; unavailable Project context closes and hides the entire dock while retaining any already-visited panes for a safe return. Design Studio's canvas/Konva internals and dedicated inspector remain unchanged.
