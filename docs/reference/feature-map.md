...
| Safe self-update foundation | gated (`PROXIMA_FEATURE_SAFE_SELF_UPDATE`, off) | external `apps/safe_updater`, `routes/self_updates.py`, `safe_updates.py`, migration 43 | disabled status only | `self_update_runs` (projection only) | root-owned journal/pointers/fence; systemd/launchd contracts; unmanaged fails closed; no activation in group 14 |
