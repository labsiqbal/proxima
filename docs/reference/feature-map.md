...
| Safe self-update foundation | gated (`PROXIMA_FEATURE_SAFE_SELF_UPDATE`, off) | external `apps/safe_updater`, `maintenance_status.py`, `routes/self_updates.py`, `safe_updates.py`, migration 43, ADR-0008, `adding-safe-updater-adapter.md` | disabled status only | `self_update_runs` (projection only) | authenticated verified-file-set publication; pinned candidate traversal; fresh controller-owned immutable inodes; durable directory creation; external-authority-first projection; read-only fence client; capability-free candidate proof; adapters unmanaged; no activation in group 14 |
