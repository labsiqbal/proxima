...
| **Durable Task delegation & dependencies** (Master slice 2A) | active | `task_delegation.py` (`TaskDelegationService`), `routes/work.py` (create/start/approve/reject/delete), `routes/graph.py` (approve), `alpha_runtime.py` (batch dispatch), `run_advancers.py` + `graph_advancers.py` (`prerequisite_changed` + landing status), `main.py` (`resume_committed` at startup), migrations 29+30 | `TaskWorkspace.tsx` (blocked-reason banner), `types.ts` (`delegation`/`dependencies`) | `task_delegations`, `task_dependencies`, `jobs.blocked_reason` | Workflows & Jobs, Alpha, repo worktrees, Container/Areas, run lifecycle |
