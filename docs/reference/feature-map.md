...
| Container Fleet, Areas & physical Ops storage | active | `container_registry.py` (projection, Live-state Fleet query, root resolver + legacy-Ops migration), `routes/containers.py` (public Fleet/detail/Areas), `project_areas.py`, `routes/projects.py` (compatibility readers + Area mutation), migrations 18 + 28 | `api/containers.ts` + canonical `Container` type; current UI remains on the `Project` compatibility type | `projects`, `project_areas`, `container_registry`, `container_ops_migrations`, live joins over `jobs`, `sessions`, `attention_items` | Master spine consumes Fleet next; graph freshness remains unavailable until the graph slice |
