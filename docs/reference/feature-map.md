...
| Container Areas & physical Ops storage | active | `container_registry.py` (root resolver + legacy-Ops migration), `project_areas.py`, `routes/projects.py` (areas endpoints), migrations 18 + 28 | (no UI yet; payload carries `code_areas`/`ops_area`) | `project_areas`, `container_registry`, `container_ops_migrations` | repo-job worktrees (slices 2+4, live); slicer job→target binding is slice 3 |
