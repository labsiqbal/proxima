...
- *Workflows & Jobs — graph engine foundation (ADR-0001)* — schema, version/run-attempt guarded state, isolated dispatch, typed advancement, and feature-gated graph job/correction routes have landed. Inspect/edit-output/rerun/approve are per-node; correction marks all transitive descendants `stale` before sequential redispatch. Default `PROXIMA_FEATURE_WORKFLOW_GRAPH=0` keeps every route and queued `wf_node` inert; no graph UI exists yet. `list_jobs` still filters to `engine='linear'`, so graph jobs cannot reach the classic Activity screen.
