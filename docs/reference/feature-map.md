...
| Files / Tree / Uploads / reference index | active | `file_targets.py`, `routes/files.py` (literal container-relative paths, prune #138, + explicit read-only Container-root inspection), `fsapi.py`, `target_preview.py`, `cf_hostnames.py` | `api/files.ts`, `fsAdapter.ts` (`projectFs` / `containerInspectionFs`), `WorkspaceTree.tsx`, `FileEditor.tsx`, `ToolDock.tsx`, `useProjectMentionItems.ts` | `projects`, `project_areas` (+FS); `turn_file_journals.root_semantics` for restore | chat, workflows, studios, artifacts, deliverable records, apps, Ops migration recovery inspection |
