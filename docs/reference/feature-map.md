...
| Safe self-update foundation | gated (`PROXIMA_FEATURE_SAFE_SELF_UPDATE`, off) | external `apps/safe_updater`, `routes/self_updates.py`, `safe_updates.py`, migration 43, ADR-0008, `adding-safe-updater-adapter.md` | disabled status only | `self_update_runs` (projection only) | fresh controller-owned immutable release inodes; POSIX/Windows durability; exact manifest/provenance tree and lockfile verification; durable active-run journal; effective candidate-identity ancestry probe; root-owned pointers/fence/backups/service config; adapters unmanaged; no activation in group 14 |
