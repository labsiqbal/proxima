...
> this doc adds the systematic code-location + relations + status grid.
>
> Snapshot: v1.0.0 · 2026-07-12. An explorable version is published as the **Proxima Feature Map** artifact.

**Status legend** — `active` shipped & wired · `dead` code/column present but not reached by live UI · `risk` active but has a known consistency hazard (see notes). (The former `gated` status died with the feature-flag system - prune A2, #129.)
...
| Health / Config | active | `main.py` (`/api/health`, `/api/config`) | `api/platformSupport.ts`, `api/files.ts` (`getPublicConfig`) | — | apps_domain + platform support bootstrap |
| Projects & FS Linking | active | `routes/projects.py`, `project_browse.py`, `directory_handles.py`, `container_registry.py` (path identity + binding state), `container_relocate.py` (relocate/rebind), migration 44 | `components/projects/FolderLinker.tsx` (link + rebind modes), `components/projects/RelocateProject.tsx`, `screens/WorkspaceOnboarding.tsx`, `screens/ProjectsScreen.tsx`, `api/projects.ts` | `projects` (`path_identity`) | files, tasks, wiki, apps, workflows |
| Container Fleet, Areas & physical Ops storage | active | `container_registry.py` (projection, Live-state Fleet query, root resolver + legacy-Ops migration orchestration), `container_activity.py` + `activity_guardian.py` (mutation/activity leases, process-tree guardians), `ops_filesystem.py` + `ops_publication.py` (no-follow identity + descriptor publication), `process_containment.py` (tree proof helpers), `routes/containers.py` (public Fleet/detail/Areas), `project_areas.py`, `routes/projects.py` (compatibility readers, Area mutation, ops-migration inspect/validate/retry), migrations 18 + 28 + ops-migration marker shape | `api/containers.ts` + canonical `Container` type; `OpsMigrationDetail.tsx`, Attention + Project Settings recovery route; current UI remains on the `Project` compatibility type | `projects`, `project_areas`, `container_registry`, `container_ops_migrations`, live joins over `jobs`, `sessions`, `attention_items` | Fleet stays independent of graph availability; scoped graph state has its own route; ownership policy in ADR-0030 |
...
| Master persistence identity | active | `master_persistence.py`, migration 31, `master_runtime.py` provisioning, `routes/master.py` + deprecated `routes/alpha.py` import alias | `MasterScreen.tsx`, `api/master.ts` legacy reader | `profiles.system_kind`, `sessions.mode`, `jobs.origin_master_session_id` | one hidden profile + project-unbound session; in-place Alpha compatibility |
...
| Bridge → Design Studio | active | `api/files` designFromImage | `ChatThread.tsx` | Design Studio |
| Media generation (`/image`, `/video`) | active | `routes/chat.py` (`_maybe_complete_chat_media`, `_save_chat_media`), `image_providers.py` + `video_providers.py` over `media_providers.py` | `Composer.tsx` (Generate) | Design Studio |
...
| Repo-job worktrees + review/merge UI | active | `worktrees.py`, `routes/work.py` (start/diff/approve/reject/delete), `routes/graph.py` (plan start/approve), `worker.py` (cwd seam), migrations 19+20 | `components/tasks/ChangesReview.tsx` + `diff.ts` (review surface), `ActivityScreen.tsx` (plan expanding row), `TaskWorkspace.tsx` (full-width page), `GraphScreen.tsx` (approve label) | `job_worktrees`, `jobs.target_area_id`, `jobs.rejected_reason`, `project_areas` | work-container areas, run lifecycle, slice-5 continuation, slice-12 satpam (consumes review states) |
...
| **Cron Scheduling** | **risk** | `schedule_policy.py`, `routes/work.py`, `scheduler.py`, `main.py` loop, migration 54 | `GraphScreen.tsx` (Availability / Automation / Run), `components/workflows/ScheduleManager.tsx`, `api/schedules.ts` | `schedules` (+ `timezone`, durable `input` bindings), `jobs`, `workflows` | workflows/jobs |
...
| Files / Tree / Uploads / reference index | active | `file_targets.py`, `routes/files.py` (literal container-relative paths, prune #138, + explicit read-only Container-root inspection), `fsapi.py`, `target_preview.py` (sandboxed preview, ADR-0042), `cf_hostnames.py` | `api/files.ts`, `fsAdapter.ts` (`projectFs` / `containerInspectionFs`), `WorkspaceTree.tsx`, `FileEditor.tsx`, `ArtifactsScreen.tsx` (recovery inspection), `useProjectMentionItems.ts` | `projects`, `project_areas` (+FS); `turn_file_journals.root_semantics` for restore | chat, workflows, studios, artifacts, deliverable records, apps, Ops migration recovery inspection |
| App Run & Preview | active | `apprunner.py`, `preview_output.py`, `preview_output_broker.py`, `preview_proxy.py`, `cf_hostnames.py`, `routes/files.py` | `AppRunner.tsx` (dock controls), `AppViewport.tsx` + `appPreview.ts` (main window) | `projects` (+in-mem) | preview-auth cookie, Cloudflare |
| In-browser Terminal | active | `terminal.py`, `routes/chat.py` (`/ws/terminal`) | `TerminalTabs.tsx`, `TerminalView.tsx` | — | projects (cwd) |
| **Artifacts** (live scan for chat cards / iterate Result) | active | `artifacts.py`, `file_targets.py`, `routes/files.py`, `run_outputs.py`, `worker.py` | `ArtifactsScreen.tsx` (routes an open: `fileKind.ts` → `DocumentEditor.tsx` or `ArtifactViewer.tsx`), `ChatThread.tsx` (ResultCards), `IterateStage.tsx` | `messages.output_links` (+FS) | canonical file targets, run outputs, studios, apps |
| **Deliverables: durable record ledger on Artifacts** (slice 8 T4; merged into one destination by prune Part D, #139; that destination is Artifacts, ADR-0043) | active | `artifact_registry.py`, `file_targets.py`, `routes/archive.py` (list + `missing` history filter + `/badges`), `run_outputs.py` (feed seam), `routes/work.py` + `routes/graph.py` (approve sync), migration 23 (seed) + v61 (container-relative record paths) | `ArtifactsScreen.tsx`, `ArtifactThumb.tsx`, `DeliverablesLens.tsx`, `ArchiveRecordPage.tsx`, `ArtifactViewer.tsx` | `artifact_records` | canonical file targets, run outputs, jobs (one status two doors), script steps (`script-output` type) |
| Design Studio / Image gen / Moodboard | active | `routes/design.py`, `moodboard.py`, `image_providers.py`, `design_scenes.py`, `higgsfield.py` | `DesignStudio.tsx`, `components/design/*` | `app_settings` (+FS: `artifacts/design`, `artifacts/moodboard`) | features gate, artifacts, wiki/run preamble |
...
- *Workflows & Jobs — graph engine (ADR-0001)* — schema, isolated typed dispatch, correction routes, chat architect, and dedicated SVG graph canvas are shipped and enabled by default. Promotion emits a normalized DAG for queued human plan review; the canvas edits nodes/dependencies, explicitly starts execution, polls live node state, and exposes correction/rerun/approval/save-template actions. Corrections mark all transitive descendants `stale` before sequential redispatch. Legacy linear rows remain readable.
- *Cron Scheduling* - the per-minute claim is an atomic conditional update keyed by local wall time plus IANA timezone; overlapping scheduler ticks cannot claim the same schedule minute twice. `schedule_policy.py` is the shared trust boundary: create/update/Run now/migration/tick resolve only durable bindings, refuse On when required inputs are unresolved (`schedule_missing_sources`), and never take the manual Run intake. Detail: [workflow-graph.md](../workflow-graph.md#scheduling-a-graph).
...
- *Artifacts / Deliverables* - chat cards and the Iterate Result keep path-oriented JSON (`produced_artifacts`/`output_links`, guarded by compare-and-swap) with a server-validated canonical file target on each returned link; paths are container-relative real paths (#139). The Deliverables tab on Artifacts is backed by the durable `artifact_records` registry (slice 8, T4): one row per producer-created deliverable version with lineage, the single two-door approval status, and a `file_missing` flag instead of vanishing records when files move or are deleted (the history filter). Workspace discovery alone does not create records.
...
- *Design Studio* — **cleanest isolation pilot**: its own modules do zero core-table writes; coupling is only the additive `sessions.mode='design'` + `runs.kind` columns. Always on. Image generation needs an image provider (default `codex` needs a working `codex login`); editing scenes needs no provider. Video generation (`/video`, #148) is a sibling of image generation and needs an OpenAI-compatible video endpoint configured in Settings. The former Video Studio/editor has been removed.
