...
| Safe self-update candidate gate | request/run projection gated (`PROXIMA_FEATURE_SAFE_SELF_UPDATE`, off); candidate proof is controller-only | `apps/safe_updater/{build,candidate,candidate_data,evidence,fixture_assembler,probe_runner,sandbox,trusted_probes}.py`, `maintenance_status.py`, `routes/self_updates.py`, `safe_updates.py`, migration 43, ADR-0008 | no safe-update UI; candidate-only identity health fields | `self_update_runs` (projection only) | fixed offline lock/build/test/doc manifest; backup-API raw clone and clone-only migration; fixture DB and isolated workspace/runner home; trusted probe hash pinning and immutable evidence; candidate startup has no migrations/background writers; adapters unmanaged and no activation |
