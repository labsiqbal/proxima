...
| Container Fleet, Areas & physical Ops storage | active | `container_registry.py` (projection, Live-state Fleet query, root resolver + legacy-Ops migration orchestration), `container_activity.py` + `activity_guardian.py` (mutation/activity leases, process-tree guardians), `ops_filesystem.py` + `ops_publication.py` (no-follow identity + descriptor publication), `process_containment.py` (tree proof helpers), `routes/containers.py` (public Fleet/detail/Areas), `project_areas.py`, `routes/projects.py` (compatibility readers, Area mutation, ops-migration inspect/validate/retry), migrations 18 + 28 + ops-migration marker shape | `api/containers.ts` + canonical `Container` type; `OpsMigrationDetail.tsx`, Attention + Project Settings recovery route; current UI remains on the `Project` compatibility type | `projects`, `project_areas`, `container_registry`, `container_ops_migrations`, live joins over `jobs`, `sessions`, `attention_items` | Fleet stays independent of graph availability; scoped graph state has a separate gated route; ownership policy in ADR-0030 |
...
| Safe self-update (unhooked) | dead - the live app no longer references the stack; `apps/safe_updater/`, `trusted-probes/safe-update/`, `infra/safe-updater/` remain on disk pending deletion (prune A1) | `apps/safe_updater/*` (unreferenced) | none | `self_update_runs` (dead projection table, kept in migration history) | ADR-0008 (historical) |
