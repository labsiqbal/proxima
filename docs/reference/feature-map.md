...
| Files / Tree / Uploads / reference index | active | `file_targets.py`, `routes/files.py` (literal container-relative paths, prune #138, + explicit read-only Container-root inspection), `fsapi.py`, `target_preview.py` (sandboxed preview, ADR-0042), `cf_hostnames.py` | `api/files.ts`, `fsAdapter.ts` (`projectFs` / `containerInspectionFs`), `WorkspaceTree.tsx`, `FileEditor.tsx`, `ToolDock.tsx`, `useProjectMentionItems.ts` | `projects`, `project_areas` (+FS); `turn_file_journals.root_semantics` for restore | chat, workflows, studios, artifacts, deliverable records, apps, Ops migration recovery inspection |
...
| **Deliverables: durable record ledger on Files** (slice 8 T4; merged into Files by prune Part D, #139) | active | `artifact_registry.py`, `file_targets.py`, `routes/archive.py` (list + `missing` history filter + `/badges`), `run_outputs.py` (feed seam), `routes/work.py` + `routes/graph.py` (approve sync), migration 23 (seed) + v61 (container-relative record paths) | `FilesScreen.tsx`, `DeliverablesLens.tsx`, `ArchiveRecordPage.tsx`, `ArtifactViewer.tsx` | `artifact_records` | canonical file targets, run outputs, jobs (one status two doors), script steps (`script-output` type) |
