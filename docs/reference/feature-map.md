...
| Feature | Status | Backend | Frontend | Tables | Relates to |
| --- | --- | --- | --- | --- | --- |
| Workflows & Jobs | active | `routes/work.py`, `workflows.py`, `graph.py`, `graph_executor.py`, `graph_advancers.py`, `run_advancers.py` | `WorkflowsScreen.tsx`, `ActivityScreen.tsx` | `workflows`, `jobs`, `node_states`, `sessions`, `runs` | scheduler, run lifecycle |
...
- *Workflows & Jobs — graph engine foundation (ADR-0001)* — schema, version/run-attempt guarded node transitions, pure DAG/I/O contracts, sequential isolated dispatch, and typed completion advancement have landed. `graph_advancers.py` accepts text, JSON Schema values, or existing contained artifact references; invalid/blocked/failed nodes, review gates, and final completion pause the graph job in `review`. Gated `PROXIMA_FEATURE_WORKFLOW_GRAPH=0`, **inert**: no graph routes/UI exist yet and queued graph runs are rejected before runner setup while disabled. `list_jobs` still filters to `engine='linear'`, so graph jobs cannot reach the classic Activity screen.
