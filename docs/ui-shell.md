...
within the owning allowed root. If none is readable, the current selection stays
...
For shell changes, run `npm --prefix apps/web test`, `npm --prefix apps/web run build`, and `git diff --check`. Tests should cover navigation order and feature-off gating, tool-rail open/close with Terminal persistence, asynchronous task success/failure, declared schedule inputs, cron grammar, and keyboard resizing. `npm --prefix apps/web run test:accessibility` first runs the focused project-link API regressions for corrective ownership, readable-ancestor selection, explicit no-ancestor failure, and configured-root jailing. It then runs the password and folder flows in a disposable real-browser fixture, records accessibility-tree, genuine Enter/Space activation, every supported theme, Lighthouse, and [screenshot evidence](evidence/auth-onboarding-accessibility/README.md) without touching live data. The harness uses an allowlisted child environment, redirects every writable Proxima path into the fixture, and disables workers, credential refresh, update checks, preview relays, and external graph egress. Its theme matrix checks title, subtitle, entered value, placeholder, error, button, input focus, and button focus styles against the rendered backgrounds for every canonical theme. It discovers the current root Tailscale Serve entry that proxies to loopback port 8765. Any `PROXIMA_A11Y_REMOTE_BASE` or `PROXIMA_A11Y_REMOTE_ADDRESS` override must match that current device and Serve mapping. The remote pass sends one unauthenticated root GET, forwards only allowlisted same-origin static shell files, fulfills config, setup status, and failed session resume inside the browser fixture, and blocks all other API, auth, cross-origin, and non-static requests. It never logs in and retains only a redacted origin label, pass state, and redacted current-device Serve provenance. Browser QA should also check authenticated desktop and narrow layouts, zoom, and reduced motion; if remote authentication prevents inspection, record that rather than using credentials.
