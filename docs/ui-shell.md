...
mutual symlink cycles. Failed configured roots retain stable lexical ownership, preventing
their selections from falling back to another configured root. New-folder validation uses
the target filesystem's encoded component-byte limit, then creates the child relative to
an open descriptor for the validated parent. Component or encoding failures return to the
folder-name field, while parent descriptor or location failures return to the
selected-folder control. If no ancestor is readable, the current selection stays
explicitly invalid until the owner retries or chooses another folder.
