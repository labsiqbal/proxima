...
Ops Home is a focused launcher with no destination dashboard grid. Its integrated Task Composer combines a searchable Project/folder picker, Agent, Add menu for attachments/image/design, Guarded or Autonomous execution policy, and the start action. Code-only Normal/Brainstorm/Debate controls are absent. `/image` and feature-gated `/design` create real media runs that are linked back to the durable task lifecycle. A created task opens `#task/<id>` with live progress, review, approval, and deliverables. Ordinary start failures clean up the queued task; media link failures preserve and identify the task for inspection.
