...
mutual symlink cycles. New-folder validation uses the target filesystem's encoded
component-byte limit; component or encoding failures return to the folder-name field,
while full parent-path failures return to the selected-folder control. If no ancestor is
readable, the current selection stays explicitly invalid until the owner retries or
