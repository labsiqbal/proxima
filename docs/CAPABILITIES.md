...
reporting a run safe. Group 16 adds a disabled, disposable-fixture transaction harness that records fence, drain, stopped-service, WAL checkpoint, final backup, staged-image, sidecar, pointer, read-only soak, writable-proof, and last-good boundaries. It accepts only the in-memory disposable service adapter, an explicitly initialized temporary fixture root, and disjoint role-confined fixture paths. The controller lock covers promotion and recovery; faults before an acknowledged last-good append restore the sealed database and both prior pointers, while unacknowledged journal tails latch fail closed and post-commit faults resume the candidate or latch the breaker. A persisted breaker or interrupted-breaker marker is authoritative over a valid-looking journal tail. Controller activation publishes a durable pending marker and takes an exclusive ingress lock; HTTP, project-app and preview proxies, terminal WebSocket, and SQLite write ingress hold shared leases until admitted side effects finish. New ingress fails closed while the controller drains existing leases. Connections configured for dynamic fencing disable statement caching, while ordinary connections retain it and skip fence checks for read-only opcodes. Fenced reads avoid profile and relay mutations or PATH shim creation, normal first-use wiki reads still seed `index.md` under a lease, and authenticated read-only session resume remains available. It is exercised by real WAL/SHM quarantine, rollback, lock-contention, hostile-path, breaker-write, statement-cache, ingress-drain, terminal-transition, and interruption-boundary fixtures only; it cannot enroll an updater, control a real service, switch a live release, replace live data, or remove a production fence. Activation remains unavailable.
