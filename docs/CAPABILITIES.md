...
Tasks. A process-local tick mutex and the Task service's database claim make repeated
and concurrent ticks safe. Satpam remains the sole steer/restart authority.
...
deltas are never copied. Restart reconciliation safely retries missing projections
without creating a second message or event. No projection can approve review,
