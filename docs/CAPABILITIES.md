...
+ **Shared shell safe areas and labels.** Mobile Menu, Back, mode, Running, Attention, Search, and New chat controls occupy independent header slots. Narrow Work widths use a second action row and reserve the Attention slot while empty, preserving pointer placement. The minimum hit-target and focus-ring tokens apply to every visible control. Work, Work project, and Recent chats use shared label tokens that clear the 4.5:1 text contrast floor.
