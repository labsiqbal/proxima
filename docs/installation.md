...
headless-browser checks. Frozen evidence is revalidated during recovery. The Group 16 model can run only with an in-memory disposable test-service adapter, an explicitly initialized empty fixture root beneath the system temporary directory, and disjoint role-confined fixture paths. It exercises dynamic HTTP, project-app and preview proxy, terminal WebSocket, and uncached SQLite write fencing, bounded drain, WAL checkpointing, single-file images, sidecar quarantine, both-pointer rollback, read-only soak, writable proof, acknowledged post-commit resume, and fail-closed ambiguous-journal handling without touching an installed service or live runtime. Neither path can switch a release, start an enrolled service, touch the live database, or remove a production fence. Do not set candidate-only environment variables manually.
